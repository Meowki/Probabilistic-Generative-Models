...
Assume that $\mu$ is nonatomic. Whenever the Monge problem admits a solution, the Kantorovich formulation is equivalent to it. More precisely, if there exists an optimal transport map \(T:\cX\to\cY\) pushing \(\mu\) onto \(\nu\), then the associated transport plan
\[
\pi_T := (\mathrm{id},T)_\#\mu \in \Pi(\mu,\nu)
\]
