...
The model for the observed and latent variable can be expressed as $p(x,z) = p(x\mid z)p(z)$. Since the conditional densities $p(x_n|z_{nk}) = \prod_{k=1}^K (p_k(x_n))^{z_{nk}}$, we can express the \textbf{complete-data likelihood} by
\begin{align}
	l_c(\theta) 
	&= \log \prod_{n=1}^N\prod_{k=1}^K (\pi_kp_k(x_n))^{z_{nk}} \nonumber \\
	& = \sum_{n=1}^N\sum_{k=1}^K z_{nk} (\log  \pi_k + \log  p_k(x_n)).
\end{align}
...
A workaround to this is to replace the cluster assignments $z_{nk}$, by their conditional expectations $\gamma_{nk} = \E{z_{nk}|x_{n}, \theta}$. That is, 
\begin{align}
	\gamma_{nk}
	&= 1*\P(z_{nk} = 1 |x_n, \theta) + 0*\P(z_{nk} = 0 |x_n, \theta)\nonumber \\ 
	&= \frac{\pi_k p_k(x_n \mid \theta)}{\sum_{j=1}^{K}\pi_j p_j(x_n \mid \theta)}.
\end{align}
Then, we can perform an iterative procedure by: i) optimising $l_c(\theta)$ using $\gamma_{nk}$ above, and ii) computing $\gamma_{nk}$ using $\theta^\star=\argmax \mathbb{E}_{z\mid x,\theta} [l_c(\theta)]$. 
...
\begin{example}[KL versus $L_1$]
	...
	\begin{align}
		\DL{p}{q_n} &= \int_0^{1/n} |e^{-n}-1|\dx + \int_{1/n}^1 |c_n -1|\dx = \frac{|e^{-n}-1|}{n} + \frac{(n-1)|c_n-1|}{n}\\
		\KL{p}{q_n} &=  \int_0^{1/n} -\log e^{-n} \dx + \int_{1/n}^1 -\log c_n \dx =  1 + \frac{-(n-1)}{n}\log c_n.
	\end{align}
...
\begin{example}[Asymmetry of the KL between two Gaussians]
	The KL divergence between two Gaussians is
	\begin{equation}
		\KL{\cN(\mu_0,\sigma_0^2)}{\cN(\mu_1,\sigma_1^2)} = \log\frac{\sigma_1}{\sigma_0} + \frac{\sigma_0^2 + (\mu_0-\mu_1)^2}{2\sigma_1^2} - \frac{1}{2}. \label{eq:2Gaussians}
	\end{equation}
