...
\missing{Example: hierarchical Gaussian mixture}
\missing{Example: Bayesian linear regression}
\missing{Example: State space models}
...
\]
This means that if the chain starts at $z \sim p(z \mid x)$, then after one step the chain remains in the same distribution. 

Second, the chain should converge to its stationary distribution. 

\missing{A necessary condition for this is detailed balance. Also refer to irreducibility and aperiodicity}
