...
where recall that the symbol $\propto_\theta$ denotes proportionality up to terms independent of $\theta$.
 ...
 \begin{itemize}
    \item The denoising model $\mu_\theta(\x_t,t)$ is trained to match the variational denoiser $\tilde\mu_t(\x_t,\x_0)$ in eq.~\eqref{eq:conditional_denoiser_mean}, meaning that $\mu_\theta(\x_t,t)$ should be designed with that objective in mind. 
    \item Note that both terms depend on the noisy sample $\x_t$, which can be expressed according to eq.~\eqref{eq:noisy_sample_from_x0} as $\x_t = \sqrt{\bar\alpha_t}\x_0 + \epsilon\sqrt{1-\bar\alpha_t}$, for $\epsilon\sim\cN(0,\I)$.
    \item As a consequence, the objective can be rewritten wrt a netwrok that predict the noise variable $\epsilon$
 \end{itemize} 
 \missing{clarify the steps above in more detail}
