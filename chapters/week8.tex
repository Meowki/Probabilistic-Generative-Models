...
\phi_0(\x)=\x.
\]
For a particle $\x\sim p_0(\x) = p(\x)$, the trajectory $t \mapsto \phi_t(\x)$ describes the evolution the particle under the velocity field $\v_t$, where the density of $\x$ evolves along the prescribed probability path
$\{p_t\}_{t\in[0,1]}$. 
...
Therefore, $p_t(\x)$ and $\v_t(\x)$ satisfy the continuity equation, which implies that $\v_t(\x)$ generates the marginal probability path. The interchange of derivatives and integrals above is justified under standard regularity assumptions (e.g., smooth vector fields and integrable densities).
\end{proof}
...
\end{align}
ensuring that the conditional probability path goes from a standard normal on $\R^D$ to a Gaussian concentrated on $\x_1$. Among several alternatives, we choose a flow that generates that probability path in eq.~\eqref{eq:conditional_Gaussian_PP} given by 
\begin{equation}
...
\begin{equation}
    p_t( \x \mid \z) = \cN(\x \mid t\x_1 + (1-t)\x_0,\sigma^2). 
\end{equation}
with the associated velocity field
\begin{equation}
    \v_t( \x \mid \z) = \x_1 - \x_0.
\end{equation}
Here, assuming $\sigma<<1$, the conditional probability path evolves from a Gaussian concentrated on a sample from the source distribution $\x_0$ to a data sample $\x_1$. 
...
\begin{remark}
    ...
    geometry of the two distributions. Instead, one can choose
    $q(\z)=\pi(\x_0,\x_1)$ to be the OT coupling between the
    (empirical) source and target distributions, yielding straighter trajectories
