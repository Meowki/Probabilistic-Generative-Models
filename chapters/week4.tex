...
k(x,x').
\]
As a consequence, implementing a GP with the kernel in eq.~\eqref{eq:poly_kernel} us equivalent to implementing a (Gaussian) Bayesian linear regression model with the feature in eq.~\eqref{eq:poly_phi}.
\end{example}
...
\end{equation}
Under the SE kernel, the correlation between two points decays smoothly and long-range correlations are difficult to model due to the tail behaviour of the Gaussian. The parameters of the SE as the \emph{lengthscale} $l>0$, which controls de memory or correlation length of the process, and the marginal variance $\sigma^2$, which controls the amplitude (or power) of the trajectories. 
...
\end{equation}
is obtained by adding infinite SE kernels with different \textit{lenghtscales}\footnote{
    Denoting $r = x-x'$, $k_{RQ}(r)
    =
    \int 
    k_{SE}(r|\tau) \tau^{\alpha-1}\exp(-\alpha\tau/\beta)\d\tau
    $, where $\tau = l^{-2}$ and $\beta^{-1} = l^{2}$.}, where $\alpha$ controls the spread of the considered lengthscales, and $l$ is a sort of reference lengthscale.
...
Since a GP is a generatie model for infinite-dimensional functions, sampling form it might seem impossible at first. However, recall that a GP defines a joint Gaussian distribution over the function values \emph{at any finite collection of input points}. See Fig.~\ref{fig:gp_graphical_model_full} for a graphical model representation.
...
\begin{align}
    m_\ast &= k_\ast^\top (K+\sigma^2 I_N)^{-1}\y,\label{eq:GP_post_pred_mv}\\
...
Exploring the posterior mean and variance in eq.~\eqref{eq:GP_post_pred_m}-\eqref{eq:GP_post_pred_v}, observe that. First, the posterior mean is a linear combination of the observations, with the weights given by the covariance between training and new samples. Second, the posterior variance features two terms: the first is the prior variance and the second one is  a strictly-negative term that's proportional to the covariance between the training and new points.  

Algorithm \ref{alg:gp_posterior} presents a a general procedure for prediction using GPs.  
