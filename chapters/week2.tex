...
We will introduce a learning approach for PGMs that feature a latent variable called Expectation Maximisation (EM). We will first present it in the particular case of the GMM model, and then in the general case. 
...
Recall our notation involving a an observed variable $x$ and a latent variable $z$. In general LVMs, the log-likelihood can be expressed as 
\begin{equation}
...
The likelihood related to the complete dataset is 
\begin{align}
    \log p(\x,\z|\theta) 
